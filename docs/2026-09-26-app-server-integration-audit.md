...
- Carpetas, asignación de recetas, favoritos, etiquetas personales, preferencias de presentación y lista de compra viven en almacenamiento local por UUID. El servidor solo guarda la biblioteca y etiquetas culinarias del contenido; no hay API/tablas de sincronización de organización personal. Otro dispositivo o instalación no los recupera del servidor; restauración desde una copia de seguridad del dispositivo no se verificó. Es un límite funcional pendiente si se espera sincronización cloud.
...
- Suite servidor + contratos cruzados iOS: 246 pasaron, 2 omitidas deliberadamente porque están reemplazadas por tests de comportamiento del ClientStateHarness. Se comprueban campos `free_used_this_year`, `client_delivery_id`, `error_code`, el orden de persistencia de ShareInbox y la purga de datos locales al borrar cuenta. Los tests cruzados antes se omitían por una ruta fija `IosAPP/`; ahora localizan el checkout hermano `ReciApp-iOS` o `RECIAPP_IOS_ROOT`. El entorno local no tiene `sentry_sdk`; para ejecutar suite se usa shim temporal sin cambios en el repositorio, así que telemetría Sentry no queda validada.
